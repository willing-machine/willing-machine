...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Willing-Machine thinks for you about the meanings.",
        epilog="Example:  willing-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | willing-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | willing-machine         #

    $ willing-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="willing-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"willing-machine: {utterance}.")
        syslog.closelog()
